// TODO: Recreate the force vis on filter
// TODO: Inline docs
	...
	network.VisFunc = function() {
		...
		var links = network.SVG.selectAll(".link")
			...
			.attr("class", function(d, i) {
				...
				// return dashed 
				return "" + " link e s s" + d.source.id + " t t" + d.target.id;
			})
			.call(drag);
		...
		network.SVG.updateNodes = function() {
			...
			network.Scales.nodeSizeScale = Utilities.makeDynamicScale(
				...
				network.config.meta.nodes.styleEncoding.size.attr,
				"linear",
				network.config.meta.nodes.styleEncoding.size.range
			);
			...
			network.SVG.nodes.selectAllToleranceFiltered(true)
				.attr("r", function(d, i) {
					return network.Scales.nodeSizeScale(d[network.config.meta.nodes.styleEncoding.size.attr]);
				})
			...
			network.SVG.selectAll("text")
				...
				.style("font-size", function(d, i) {
					return network.Scales.nodeTextScale(d[network.config.meta.nodes.styleEncoding.size.attr]);
				})
				.text(function(d, i) {
					if (d[network.config.meta.nodes.styleEncoding.size.attr] > network.Scales.nodeSizeScale.domain()[1] * network.config.meta.labels.styleEncoding.displayTolerance) {
