visualizationFunctions.ComponentEdgeWidthLegend = function(element, data, opts) {
	...
	network.VisFunc = function() {
        ...
        network.SVG.edgeTitle = network.SVG.append("text")
            .style("color", "black")
            .attr("class", "l2")
            .attr("text-anchor", "start")
            .attr("x", "10%")
            .attr("y", "80%")
            .call(wrap, network.config.dims.fixedWidth - 10, network.parentVis.config.meta.edges.prettyMap[network.parentVis.config.meta.edges.styleEncoding.opacity.attr] || network.parentVis.config.meta.edges.styleEncoding.opacity.attr)
