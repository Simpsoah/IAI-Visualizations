visualizationFunctions.ComponentNodeColorLegend = function(element, data, opts) {
	...
	network.SVG = d3.select(element[0])
		...
		.attr("transform", "translate(" + (network.config.dims.fixedWidth * .25) + ",0)")
	...
	network.VisFunc = function() {
		...
		network.SVG.append("text")
			...
			.text(Utilities.format(network.parentVis.Scales.nodeColorScale.domain()[network.parentVis.Scales.nodeColorScale.domain().length - 1]))
		...
		// 	.attr("text-anchor", "start")
		$("#cncl").html(
			network.parentVis.config.meta.nodes.prettyMap[network.parentVis.config.meta.nodes.styleEncoding.color.attr] || network.parentVis.config.meta.nodes.styleEncoding.color.attr)
